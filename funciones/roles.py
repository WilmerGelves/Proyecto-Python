...
def newSpecialist():
    ...
    ***********************
    ...
    if (longitud == 0 ):
        ...
        try:
            codigo = int(input('Codigo del servicio que ofrecerá el médico: '))
            codigo= str(codigo)
            try:
                if codigo == servicio['area'][codigo]['codServicio']:
                    identMedico = int(input('Número de identificacion del médico: '))
                    identMedico= str(identMedico)
            ...
            print('Estas ingresando datos no permitidos')
            globals.pausar_pantalla()
            newSpecialist()
